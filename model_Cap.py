...
from routing import alpha_infer, beta_routing, gamma_routing
...
# beta process
    b_IJ1 = tf.zeros(shape=[1, 8, 2], dtype=tf.float32)
    b_IJ2 = tf.zeros(shape=[1, 2, 1], dtype=tf.float32)

    beta_part_body_land_outs = beta_routing(part_land_outs, part_body_land_outs, 8, 2, b_ij=b_IJ1)
    beta_full_body_land_outs = beta_routing(part_body_land_outs, full_body_land_outs, 2, 1, b_ij=b_IJ2)

# gamma process
    b_IJ3 = tf.zeros(shape=[1, 1, 2], dtype=tf.float32)
    b_IJ4 = tf.zeros(shape=[1, 2, 8], dtype=tf.float32)
    gamma_part_body_land_outs = gamma_routing(full_body_land_outs, part_body_land_outs, 1, 2, b_IJ3)
    gamma_part_land_outs = gamma_routing(part_body_land_outs, part_land_outs, 2, 8, b_IJ4)
...
####################################################11.29##############################################################
    ######## separate every node, concatenate the alpha, beta, gamma of the same node, train together ########
